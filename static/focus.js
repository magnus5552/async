...
async function run() {
  ...
  const ogrns = { ogrn: orgOgrns.join(",") };
  const [requisites, analytics, buh] = await Promise.all([
    sendRequest(API.orgReqs, ogrns),
    sendRequest(API.analytics, ogrns),
    sendRequest(API.buhForms, ogrns),
  ]);
...
function sendRequest(url, query = {}) {
  return fetch(url + "?" + new URLSearchParams(query), { method: "GET" }).then(
    (x) => {
      if (x.ok) {
        return x.json();
      }
